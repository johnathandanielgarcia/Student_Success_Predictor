...
If time allows: -> Did not finish by project due date. Plan to continue implementing after due date.
...
!!! Note: assumes all inputs are valid inputs !!!
'''
...
# turns dataframe into appropriate X and y for training model
def df_to_Xy(df, target_col):
    X = df.drop(columns = [target_col]).values
    y = df[target_col].values
    return X, y

# determine if a value is an acceptable value
def valid_option(value, acceptable_values):
    return True if value in acceptable_values else False

# maps model output (prediction) to a corresponding grade
def prediction_to_grade_dt(prediction):
    match prediction:
        case 0:
            return 'A'
        case 1:
...
# trains a decision tree model with specified parameters 
def train_decision_tree(min_sample_split=5, max_depth=10):
...
# calculate decicion tree accuracy without library
def decision_tree_accuracy(dt, X_test, y_test):
    accuracy = (dt.predict(X_test) == y_test).mean()
    return accuracy

# train decision tree with user specified values 
...
# predict on new data using decision tree
...
def train_logistic_regression(learning_rate=0.04, num_iter=1750):
...
# maps prediciton output to a pass/fail
def prediction_to_grade_lr(label):
    if label == 1:
        return "Pass (A, B, C)"
    else:
        return "Fail (D, F)"
...
def main():
    ...
    if option == 0:
        ...
        while digit != -1:
            # verify int is a valid int
            while valid_option(digit, [-1, 0, 5]) == False:
                digit = int(input("Invalid. Please enter 5, 0, or -1.\n"))

            # case for training tree, repeats until user wants a different action
            while digit == 5:
                digit = user_trained_decision_tree()

            # case for predicting, repeats until user wants a different action
            while digit == 0:
                ...
                print(f"The model predicted a grade of {prediction_to_grade_dt(label)}")
    ...
    # user chose logistic regression
    elif option == 1:
        # initial training
        print("Training...")
        ...
        # show user accuracy, ask if they want to train on custom params, predict with custom data, or exit
        print('Logistic Regression model has been trained with the following parameters:\n' \
        'learning_rate = 0.04\n' \
        'num_iter = 1750\n' \
        f'These parameters have been proven to achieve the highest accuracy of {acc:.3f}.\n')
        digit = int(input("If you wish to train the model with your own choice of parameters, please enter 5. \n" \
        "If you wish to predict on your own data, please enter 0.\n" \
        "If you wish to exit, please enter -1.\n"))
        ...
        while digit != -1:
            ...
            # case for training with custom params, repeats until user wants to do something else
            while digit == 5:
                digit = user_trained_logistic_regression()

            # case for predicting, repeats until user wants to do something else
            while digit == 0:
                which_log_model = int(input("If you would like to predict using the highest accuracy logistic regression model, please enter 0.\n" \
                "If you would like to use custom parameters, please enter 1.\n"))
                
                label = predict_logistic_regression(which_log_model)
                print(f"The model predicted the student will {prediction_to_grade_lr(label)}")
